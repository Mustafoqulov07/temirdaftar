...
export const AdminLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 dark:bg-[#070b14] dark:text-slate-100 flex flex-col pb-20 md:pb-0 font-sans antialiased selection:bg-indigo-500 selection:text-white relative overflow-hidden">
      ...
      {/* Top Header */}
      <header className="bg-white/90 dark:bg-[#0b1222]/80 backdrop-blur-xl border-b border-gray-200 dark:border-slate-800/80 sticky top-0 z-40 shadow-sm dark:shadow-2xl dark:shadow-black/40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-18 flex items-center justify-between">
            ...
            <Link to="/admin" className="flex items-center space-x-3.5 group">
              ...
              <div className="flex flex-col">
                ...
                <div className="flex items-center gap-2 text-[11px] text-slate-400">
                  ...
                  <span className="font-mono text-emerald-500 dark:text-emerald-400/90 text-[10px]">Markaziy Boshqaruv Tizimi</span>
                </div>
              </div>
            </Link>

            {/* Desktop Navigation Tabs */}
            <nav className="hidden md:flex items-center space-x-1.5 pl-6 border-l border-gray-200 dark:border-slate-800/80">
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center space-x-2 px-4 py-2 rounded-xl text-xs font-bold transition-all duration-200 relative ${
                      active
                        ? 'bg-indigo-50 dark:bg-gradient-to-r dark:from-indigo-600/30 dark:to-purple-600/20 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-500/40 shadow-lg shadow-indigo-500/10'
                  ...
                  >
                    <Icon className={`w-4 h-4 ${active ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-400 dark:text-slate-400'}`} />
                    <span>{item.name}</span>
                  </Link>
                );
              })}
          ...
          <div className="flex items-center space-x-3">
            ...
            {/* Theme toggle — admin uchun ochiq rejim ham mavjud */}
            <ThemeToggle className="text-slate-400 hover:text-amber-300 hover:bg-slate-800/60" />
            ...
            <div className="hidden sm:flex items-center space-x-2.5 bg-gray-50 border border-gray-200 dark:bg-slate-900/90 dark:border-slate-800 px-3.5 py-1.5 rounded-xl">
              ...
              <div className="flex flex-col text-left">
                ...
                <span className="text-[10px] font-mono text-gray-500 dark:text-slate-400 leading-tight">
                  {user?.phoneNumber}
                </span>
              </div>
            </div>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-[#0b1222]/95 backdrop-blur-xl border-t border-gray-200 dark:border-slate-800 z-40 h-16 flex justify-around items-center px-2">
        {navItems.map((item) => {
          const Icon = item.icon;
          const active = isActive(item.path);
          return (
